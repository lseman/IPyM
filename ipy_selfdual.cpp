...
void convert_to_standard_form(
  ...
  std::vector<double> val_ub(n_ubounds);
...
void start_linear_solver(SparseSolver &ls,
  ...
  auto insertBlock = [&](const Eigen::SparseMatrix<double> &block, int startRow,
                         int startCol) {
    for (int k = 0; k < block.outerSize(); ++k) {
      for (Eigen::SparseMatrix<double>::InnerIterator it(block, k); it; ++it) {
        tripletList.emplace_back(it.row() + startRow, it.col() + startCol,
                                 it.value());
      }
    }
  };
...
void update_residuals(Residuals &res, const Eigen::VectorXd &x,
                      ...
                      const Eigen::VectorXd &ubv, const Eigen::VectorXi &ubi,
                      const Eigen::VectorXd &vbv, const Eigen::VectorXi &vbi,
                      double tau, double kappa) {
  // Efficiently calculate rp (primal residual) using noalias to avoid
  // unnecessary allocations
  res.rp.noalias() = tau * b - A * x;
  res.rpn = res.rp.norm(); // Primal residual norm

  // Calculate ru (upper bound residual) with fewer iterations by combining
  // operations
  ...
  // Efficiently calculate rd (dual residual) using noalias to avoid temporary
  // allocations
  ...
  res.rgn = std::abs(
      res.rg); // Since rg is scalar, its "norm" is just the absolute value
}
...
run_optimization(Eigen::SparseMatrix<double> &As,
  ...
  for (int k = 0; k < max_iter; ++k) {
    ...
    // Call the function
    update_residuals(res, x, lambda, s, v, w, A, b, c, ubv, ubi, vbv, vbi, tau,
                     kappa);
    ...
    // Calculate delta_0
    delta_0 =
        regG + kappa / tau - delta_x.dot(c) + delta_y.dot(b) - delta_z.dot(ubv);

    // Call the function
    solve_newton_system(Delta_x, Delta_lambda, Delta_w, Delta_s, Delta_v,
                        Delta_tau, Delta_kappa, ls, theta_vw, b, c, ubi, ubv,
                        delta_x, delta_y, delta_z, delta_0, x, lambda, w, s, v,
                        tau, kappa, res.rp, res.ru, res.rd, res.rg,
    ...
    while ((ncor <= 3) && (alpha < 0.9995)) {
      ...
      */
      ...
      solve_newton_system(
          ...
          delta_y, delta_z, delta_0, x, lambda, w, s, v, tau, kappa,
          Eigen::VectorXd::Zero(res.rp.size()),
          Eigen::VectorXd::Zero(res.ru.size()),
          Eigen::VectorXd::Zero(res.rd.size()), 0, -t_xs, -t_vw, -t0);

      // compute max step length
      alpha_c = max_alpha(x, Delta_x_c, v, Delta_v_c, s, Delta_s_c, w,
